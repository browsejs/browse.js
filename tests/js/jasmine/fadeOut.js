describe('fadeOut', function() {
  ...
  function updateChangeRates(elem, before, changeRates) {
    ...
    var rate = (1000 * (1.0 - elem.opacity()))/(currTime.getTime() - before.getTime())
    if(!isNaN(rate)) {
  ...
  describe('linear mode', function() {
    ...
    it('should complete to get a 0% opacity invisible element', function() {
      var elem = $_(document.body).append('<div style=background-color:black;height:20px>').lastChild()
      ...
      waitsFor(function() {
        return done
      }, 'fadeOut checks should have been done', 3000)
    })
    ...
    it('should use this mode by default', function() {
      ...
      runs(function() {
        ...
        expect(spy.callback).toHaveBeenCalled()
        elem.element.parentNode.removeChild(elem.element)
        checkMeanVariance(changeRates, 1.0, 0.04)
        done = true
      })
      waitsFor(function() {
        return done
    ...
    it('should take a linear course in time', function() {
      ...
      var before = new Date(), changeRates = [ ]
      runs(function() {
        elem.fadeOut(interval, 'linear', function(obj){spy.callback(obj)})
      })
      waitsFor(function() {
        updateChangeRates(elem, before, changeRates)
        return done
  ...
  describe('easeInQuadratic mode', function() {
    ...
    it('should complete to get a 0% opacity invisible element', function() {
      ...
      var before = new Date()
      runs(function() {
        elem.fadeOut(interval, 'easeInQuadratic', function(obj){spy.callback(obj)})
      })
      waitsFor(function() {
        return done
      }, 'fadeOut callback should have been called', 3000)
    ...
    it('should take a quadratic course in time with change rate always increasing', function() {
      ...
      runs(function() {
        ...
        expect(spy.callback).toHaveBeenCalled()
        elem.element.parentNode.removeChild(elem.element)
        checkMeanVariance(changeRates, 0.7, 0.1, 0.03)
        var sum = 0
  ...
  describe('easeOutQuadratic mode', function() {
    ...
    it('should complete to get a 0% opacity invisible element', function() {
      ...
      var before = new Date()
      runs(function() {
        elem.fadeOut(interval, 'easeOutQuadratic', function(obj){spy.callback(obj)})
      })
      waitsFor(function() {
        return done
      }, 'fadeOut callback should have been called', 3000)
    ...
    it('should take a quadratic course in time with change rate decreasing post around 1/3 mark', function() {
      ...
      runs(function() {
        ...
        expect(spy.callback).toHaveBeenCalled()
        elem.element.parentNode.removeChild(elem.element)
        checkMeanVariance(changeRates, 1.5, 0.17, 0.03)
        changeRates.shift()
        changeRates.shift()
        changeRates.pop()
        var sum = 0
        ...
        for(var n = 7, idx = n * 5; idx < changeRates.length; idx += 5) {
          ...
          if(idx2 === idx + 5) {
            ...
            if('requestAnimationFrame' in window) {
              ...
              expect(sum - nextSum).toBeLessThan(0.75)
            }
            //log(nextSum)
          }
          sum = nextSum
